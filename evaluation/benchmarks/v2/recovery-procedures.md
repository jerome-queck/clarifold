...
Each candidate record must bind `initialFailureEvidence` to one exact passing named test from the scenario's `testEvidence`, including a scenario-specific failure or recovery-precondition kind and the test duration. A nonempty list of positive tests is not evidence that an initial failure or recovery precondition was observed.
