...
- `src/shared/learning-application.ts` is the public Learning Application boundary and owns Study Workspace, Study Mission, Learning Session, Managed Asset and Linked Source relationships, Source Index lifecycle and search, filing, navigation, Local Working Mode, Pending Questions, privacy-minimized external-research receipts and permissions, Learning Artifact revisions and portable-copy construction, session metadata search, and durable state transitions. `src/shared/external-research.ts` is the provider-neutral external-research contract; it remains separate from the Model Runtime boundary. Canonical application state and the clearable `source-index.json` cache are persisted separately and atomically.
