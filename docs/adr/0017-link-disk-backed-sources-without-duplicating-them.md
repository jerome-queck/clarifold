...
The ad-hoc demo package is currently a non-App-Sandbox preview, so Electron cannot create or activate Mac App Store security-scoped bookmarks there. That preview may reopen a source from its last-known path because the process is unrestricted, but it records no synthetic grant and must label the path as location metadata rather than permission. The same adapter requires and persists the opaque bookmark supplied by a sandboxed MAS runtime; release packaging must not claim sandbox-persistent access until that signed build and its provisioning are available.
