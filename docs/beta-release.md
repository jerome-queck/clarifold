...
The candidate quality report and its complete, non-private evidence are bundled under ignored `out/release/macos-beta-<version>/` only after `quality:gate:beta` passes. During the current development-preview phase, that evidence is retained as a temporary workflow artifact, not attached to a GitHub prerelease. The manually dispatched internal-candidate workflow retains the exact architecture-native archive for 30 days and its report and diagnostics for 14 days; the archive and report must target the same clean revision no older than 30 days. The deterministic harness fixture is uploaded separately by routine CI as `quality-gate-harness-fixture-only`; it is never included in or presented as the candidate report.
...
The internal-candidate workflow artifact is explicitly `internal-candidate-not-for-distribution`: unsigned, limited-support, and not a supported public production release. It is distinct from a future GitHub Release, GitHub Package, or source archive. No candidate tag or public prebuilt download is created while the deferred [public-distribution gate](adr/0035-curate-public-releases.md#deferred-public-distribution-gate) is incomplete.

On the first default launch, Clarifold checks for the prior Quick Study application-data directory. If the Clarifold destination is absent or empty, it stages and validates a copy, records a migration receipt, and activates the destination atomically. The old Quick Study directory remains unchanged as a rollback source. A populated destination, incomplete or invalid source, insufficient space, interrupted copy, permissions failure, or concurrent launch stops automatic migration and shows in-app recovery guidance; Clarifold never merges or deletes either directory. Do not run the retained Quick Study and Clarifold directories concurrently after migration because they can diverge and are not synchronized. Explicit `CLARIFOLD_DATA_DIR` and `CLARIFOLD_TEST_USER_DATA_DIR` paths, including the deprecated one-beta `QUICK_STUDY_DATA_DIR` alias, never inspect or migrate the default directories.
